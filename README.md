...
- 支持 3x-ui 3.0.1 API Token，对接新版 `/panel/api` 接口；Token 不可用时可回退账号密码登录。
- 支持从 3x-ui 实时读取 vmess/ws 入站。
- 支持节点白名单，不勾选时默认展示全部可用节点，并支持按面板分组搜索和多选。
- 支持 LDC 总兑换流量限制，`0` 表示不限制。
- 支持按“每月某日 + 具体时间（北京时间）”重置 LDC 月度额度池。
- 支持 LDC 订单查询、状态同步、退款记录和商户分发工具。
- 支持 Cloudflare Turnstile 保护前台兑换、支付、查询和后台登录。
- 支持后台管理兑换码、用户、LDC 订单、3x-ui 设置、LDC 设置和账号信息。
- 支持后台删除未被业务数据引用的 3x-ui 面板，避免误删正在使用的面板。
...
- 当前版本的 3x-ui API Token 由后台“设置中心 -> 3x-ui 面板设置”保存到数据库，不从 `.env` 直接读取。
- 首次部署时如果你还没有配置 API Token，可以先通过 `XUI_USERNAME` 和 `XUI_PASSWORD` 接入默认面板，后续再到后台补充或改成 API Token 模式。
- 如果你准备只使用 API Token，也可以先启动项目，再到后台新增一个只填写 `3x-ui 地址 + API Token` 的面板。
...
- 可以新增多个 3x-ui 面板。
- 可以启用或禁用某个面板。
- 可以为单个面板配置 API Token，优先走 Bearer Token 调用新版 `/panel/api`。
- 如果 API Token 校验失败，并且该面板同时配置了账号密码，系统会自动回退到账号密码 + CSRF 登录。
- 可以通过按面板分组的搜索下拉框勾选上架节点白名单。
- 可以删除面板，但有以下保护：
  - 至少保留一个面板，最后一个面板不允许删除。
  - 已被兑换码、用户或 LDC 订单引用的面板不允许删除。

如果你使用的是 3x-ui 3.0.1，推荐在面板后台生成 API Token 后填入兑换系统，不再依赖账号密码。
...
- 设置“每月重置日期”和“重置时间（北京时间）”，系统会在该时刻开启新的月度额度周期。
- 小月没有对应日期时，会自动按当月最后一天执行。例如设置每月 `31` 日，2 月会按 2 月最后一天重置。
- 新周期开始后，上一周期里“已支付或已完成、但尚未到期”的订阅仍会继续占用新周期额度，到订阅到期后自动释放。
- 例如将重置时间设置为每月 `21` 日 `15:12`，那么每个自然月都会在北京时间当日 `15:12` 进入新周期，不是凌晨 `00:00`，也不是当天 `24:00`。
- 待支付订单会按系统的待支付过期时间做短暂预占，避免并发超卖。
- 退款成功后不会立即释放 LDC 总流量占用；只有退款成功并禁用对应 3x-ui 客户端后，才会释放该订单占用的流量统计，避免退款后客户端继续使用造成超额。
- LDC 订单退款成功后，前台查询和订阅接口会停止返回该订单的订阅内容；兑换码订阅到期后也会停止返回订阅内容。
...
- 前台按当前激活的标签页懒加载 Turnstile 组件，减少隐藏标签页内多实例渲染失败的问题。
- 服务端只会在拿到真实公网 IP 时才把 `remoteip` 传给 Cloudflare 校验，避免本地回环或内网代理地址导致误判。
...
- 检查 `app.py` 中是否仍保留了生产环境专用的 `DEFAULT_INBOUNDS`、`SUB_CONFIG` 或其他硬编码默认值。
- 检查 Git 历史里没有误提交过真实 `LDC_KEY`、`TURNSTILE_SECRET_KEY`、3x-ui 密码。
- 确认生产环境管理员密码已修改，不使用 `admin/admin`。
- 若启用退款：只有退款成功并且对应 3x-ui 客户端禁用成功后，才会释放 LDC 流量占用统计，便于后续继续兑换。
